...
const SuperAdminUserDetailPage: React.FC = () => {
  ...
  const handleToggleCanVerify = async () => {
    if (!user) return;
    try {
      setActionLoading(true);
      const newValue = !user.can_verify;
      const updated = await usersApi.toggleCanVerify(user.id, user.can_verify);
      setUser({ ...user, can_verify: updated.can_verify ?? newValue });
      toast.success(`Verify permission ${newValue ? 'enabled' : 'disabled'}`);
    } catch (error) {
      toast.error('Failed to toggle verify permission');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  if (!user) {
    return (
      <div className="bg-surface rounded-2xl border border-border p-8 text-center">
        ...
        <Button variant="outline" className="mt-4" onClick={() => navigate('/super-admin/users')}>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Button variant="outline" size="icon" onClick={() => navigate('/super-admin/users')}>
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" onClick={() => navigate(`/super-admin/users/${user.id}/edit`)}>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3 pt-2">
                  ...
                  <div className="flex items-center gap-3 text-sm pt-2 border-t border-border">
                    ...
                    <span className="text-text">KYC Status: </span>
                    <Badge variant={
          ...
          <div className="bg-surface rounded-xl border border-border p-6">
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <p className="text-sm font-medium text-text">Can Verify Courses</p>
                <p className="text-xs text-text-muted">Allows this user to verify course listings.</p>
              </div>
              <div className="flex items-center gap-3">
                <Badge variant={user.can_verify ? 'success' : 'secondary'}>
                  {user.can_verify ? 'Enabled' : 'Disabled'}
                </Badge>
                <Button
                  size="sm"
                  variant="outline"
                  onClick={handleToggleCanVerify}
                  isLoading={actionLoading}
                >
                  Toggle
                </Button>
                          ...
                          {enrollments.map((enrollment) => {
                            ...
                            return (
                              <TableRow key={enrollment.id}>
                                <TableCell>
                                  <Link
                                    to={`/super-admin/courses/${enrollment.course?.id}`}
