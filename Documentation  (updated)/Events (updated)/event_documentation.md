...
- **Method**: `GET`
- **Authentication**: Not Required
- **Description**: Returns a list of all upcoming events (date >= now), sorted by date ascending.

#### Success Response
**Code**: `200 OK`
Returns the same format as the List Events endpoint, but filtered to only future events.

### 3. List Past Events
- **URL**: `GET /api/events/past/`
- **Method**: `GET`
- **Authentication**: Not Required
- **Description**: Returns a list of all past events (date < now), sorted by date descending (most recent first).

#### Success Response
**Code**: `200 OK`
Returns the same format as the List Events endpoint, but filtered to only past events.
...
|---|---|---|---|
| `event_type` | `string` | No | Type of event. Defaults to `"other"`. Available options: `meetup`, `bootcamp`, `seminar`, `conference`, `workshop`, `hackathon`, `webinar`, `panel`, `networking`, `other`. |
